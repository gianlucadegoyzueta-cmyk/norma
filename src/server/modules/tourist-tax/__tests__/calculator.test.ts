...
function stay(arrival: string, departure: string | null, extra?: Partial<StayTaxInput>): StayTaxInput {
...
describe("Roma — 6€/notte, tetto 10, esente <10, surcharge Giubileo", () => {
  it("adulto, 3 notti nel 2024 (no Giubileo) = 3 × 600 = 1800", () => {
    const r = computeTouristTax(stay("2024-06-01", "2024-06-04"), [guest("a", "1990-01-01")], ROMA.rule);
  ...
  it("surcharge Giubileo 2025: 2 notti × (600+200) = 1600", () => {
    const r = computeTouristTax(stay("2025-06-01", "2025-06-03"), [guest("a", "1990-01-01")], ROMA.rule);
    expect(r.totalCents).toBe(1600);
  });

  it("tetto 10 notti: soggiorno di 12 notti tassa solo 10 (nel 2024 = 6000)", () => {
    const r = computeTouristTax(stay("2024-06-01", "2024-06-13"), [guest("a", "1990-01-01")], ROMA.rule);
  ...
  it("minore di 10 anni → esente (0), niente surcharge nemmeno nel 2025", () => {
    const r = computeTouristTax(stay("2025-06-01", "2025-06-05"), [guest("k", "2020-01-01")], ROMA.rule);
  ...
  it("esenzione per categoria (forze dell'ordine) → 0", () => {
    const r = computeTouristTax(stay("2024-06-01", "2024-06-05"), [guest("p", "1985-01-01", "FORZE_ORDINE")], ROMA.rule);
    expect(r.totalCents).toBe(0);
...
describe("Firenze — soglia esenzione 12, tetto 7", () => {
  it("11enne esente (<12), 12enne tassato", () => {
    const eleven = computeTouristTax(stay("2025-06-01", "2025-06-03"), [guest("k", "2014-01-01")], FIRENZE.rule);
    expect(eleven.totalCents).toBe(0);
    const twelve = computeTouristTax(stay("2025-06-01", "2025-06-03"), [guest("k", "2013-01-01")], FIRENZE.rule);
    expect(twelve.totalCents).toBe(1200);
  });

  it("tetto 7 notti: 9 notti → tassate 7 = 4200", () => {
    const r = computeTouristTax(stay("2025-06-01", "2025-06-10"), [guest("a", "1990-01-01")], FIRENZE.rule);
...
describe("Milano — 6,30€, soglia esenzione 18 (alta), tetto 14", () => {
  it("17enne ancora esente (<18)", () => {
    const r = computeTouristTax(stay("2025-06-01", "2025-06-03"), [guest("t", "2008-01-01")], MILANO.rule);
    expect(r.totalCents).toBe(0);
    expect(r.guests[0].exempt).toBe(true);
  });

  it("adulto, 2 notti = 2 × 630 = 1260", () => {
    const r = computeTouristTax(stay("2025-06-01", "2025-06-03"), [guest("a", "1990-01-01")], MILANO.rule);
    expect(r.totalCents).toBe(1260);
  });

  it("tetto 14: soggiorno di 20 notti → 14 × 630 = 8820", () => {
    const r = computeTouristTax(stay("2025-06-01", "2025-06-21"), [guest("a", "1990-01-01")], MILANO.rule);
...
describe("Venezia — zona + stagione + fascia 10–16 al 50%", () => {
  ...
  it("zona TERRAFERMA usa la sua tariffa (350), DEFAULT per zona ignota (400)", () => {
    const terra = computeTouristTax(stay("2025-06-01", "2025-06-02", { zone: "TERRAFERMA" }), [guest("a", "1990-01-01")], VENEZIA.rule);
    expect(terra.totalCents).toBe(350);
    const unknown = computeTouristTax(stay("2025-06-01", "2025-06-02", { zone: "ISOLA_IGNOTA" }), [guest("a", "1990-01-01")], VENEZIA.rule);
...
describe("casi limite trasversali", () => {
  it("compie gli anni DURANTE il soggiorno: età valutata all'arrivo (resta esente)", () => {
    ...
    const r = computeTouristTax(stay("2025-06-01", "2025-06-05"), [guest("k", "2015-06-03")], ROMA.rule);
    expect(r.guests[0].exempt).toBe(true);
  });
  ...
  it("arrotondamento esplicito: 633c −30% = round(443.1)=443", () => {
    const ruleWithOdd = parseTouristTaxRule({
      ...VENEZIA.rule,
      rates: [{ zone: "DEFAULT", season: { ranges: [{ from: "11-01", to: "03-31" }], modifierPct: -30 }, amountCents: 633 }],
      ageReductions: [],
    });
    const r = computeTouristTax(stay("2025-12-01", "2025-12-02"), [guest("a", "1990-01-01")], ruleWithOdd);
  ...
  it("regola senza tariffa applicabile → errore esplicito (no calcolo silenzioso)", () => {
    ...
    expect(() =>
      computeTouristTax(stay("2025-06-01", "2025-06-02", { zone: "TERRAFERMA" }), [guest("a", "1990-01-01")], ruleNoDefault),
