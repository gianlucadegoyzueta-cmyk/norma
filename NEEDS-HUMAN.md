...
- **Stato:** serializer + client pronti dove coperti; trasmissione gated fino a decisione founder.

### 5. Cron Alloggiati (invio + reconcile T+1) — ✅ IN MAIN, attivato 2026-06-27

- **Stato:** `GET /api/cron/alloggiati` in main; schedule in `vercel.json` (08:10 UTC). Env prod: `ALLOGGIATI_CRON_ENABLED=true` + `CRON_SECRET`.
- **Barriere:** (1) flag env; (2) auth Bearer cron; (3) solo credenziali con `autoSend=true` (default false). Nessun Send reale finché non attivi il toggle su una credenziale.
- **Primo Send reale:** resta decisione founder su ospite vero (guardrail #1 CRITICAL).
...
### 8. Billing Stripe — ✅ IN PROD (chiavi Vercel presenti)

- **Stato:** migrazione applicata, modulo `src/server/modules/billing` in prod. Chiavi Stripe in Vercel production.
- **Follow-up (non bloccante):** cablare `BillingGatingService.requireWriteAccess(orgId)` nelle server action di scrittura quando decidi di monetizzare. Prezzi canonici: `plan.ts` v2 (€90/€9 host).
